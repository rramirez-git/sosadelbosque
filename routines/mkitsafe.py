...
def valida_acceso(permisos=None):
    url_error = 'panel'
    url_autenticacion = 'index'
    """
    Función decoradora que verifica si un usuario esta logeado en el
    sistema y si tiene ALGUNO de los permisos pasados como parámetro
    o bien alguno de los permisos hijo

    Parámetros:

        permisos = None
            Lista de permisos a validar, en caso de recibir None sólo se
            valida que el usuario este
            logeado en el sistema
            Cada permiso debe estar en formato de cadena 'modelo.codename'
        url_error = 'panel'
            Nombre de la URL a la que se redireccionará en caso de que el
            usuario no cuente con ningún permiso
        url_autenticacion = 'login'
            Nombre de la URL a la que se redireccionará en caso de que el
            usuario no esté logeado en el sistema
    """
